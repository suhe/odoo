...
_logger = logging.getLogger('report.account.journal.entries.report.webkit')
...
class HeaderFooterTextWebKitParser(webkit_report.WebKitParser):
    ...
    def generate_pdf(self, comm_path, report_xml, header, footer, html_list,
                     webkit_header=None, parser_instance=None):
    ...
    def create_single_pdf(self, cursor, uid, ids, data, report_xml,
        ...
        parser_instance = self.parser(cursor,
                                      ...
                                      self.name2,
                                      context=context)
        self.pool = openerp.registry(cursor.dbname)
        ...
        if report_xml.report_file:
            ...
            if os.path.exists(path):
                with open(path) as file_job:
                    template = file_job.read()
        ...
        if report_xml.webkit_debug:
            try:
                ...
                raise except_osv(_('Webkit render'), msg)
            return (deb, 'html')
        bin_exec = self.get_lib(cursor, uid)
        pdf = self.generate_pdf(bin_exec, report_xml, head, foot, htmls,
